...
>    per clone: `pip install pre-commit && pre-commit install` (see
>    `.pre-commit-config.yaml`). CI also runs `gitleaks` on every PR/push to
>    `main`. If gitleaks flags anything, STOP.
...
- **Committed** gitleaks pre-commit hook (`.pre-commit-config.yaml`, rev pinned).
  Enable once per clone: `pip install pre-commit && pre-commit install`.
...
on every fresh clone, run `pre-commit install` before the first commit.
